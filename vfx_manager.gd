...
func spawn_damage_number(pos: Vector2, amount: int, is_crit: bool = false, element: String = "") -> void:
	...
	label.position = pos + Vector2(randf_range(-8, 8), -20)
	label.z_index = 100
	
	var size := 14 if is_crit else 11
	label.add_theme_font_size_override("font_size", size)
	
	var color := Color(1.0, 0.9, 0.3) if is_crit else Color(1.0, 1.0, 1.0)
	if element != "" and PALETTES.has(element):
	...
	# Animation: Hochspringen, dann fallen
	var start_y := label.position.y
	var peak_y := start_y - (30 if is_crit else 20)
	
	var tween := label.create_tween()
	tween.tween_property(label, "position:y", peak_y, 0.2).set_trans(Tween.TRANS_QUAD).set_ease(Tween.EASE_OUT)
	tween.tween_property(label, "position:y", start_y + 10, 0.3).set_trans(Tween.TRANS_QUAD).set_ease(Tween.EASE_IN)
	tween.parallel().tween_property(label, "modulate:a", 0.0, 0.2).set_delay(0.3)
	...
	if is_crit:
		...
		scale_tween.tween_property(label, "scale", Vector2(1.0, 1.0), 0.15).set_trans(Tween.TRANS_ELASTIC)
...
func spawn_gold_number(pos: Vector2, amount: int) -> void:
	...
	var label := Label.new()
	label.text = "+%d" % amount
	label.position = pos + Vector2(0, -25)
	label.z_index = 100
	...
	var tween := label.create_tween()
	tween.set_parallel(true)
	tween.tween_property(label, "position:y", label.position.y - 30, 0.6).set_trans(Tween.TRANS_QUAD).set_ease(Tween.EASE_OUT)
